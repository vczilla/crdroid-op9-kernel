...
static const struct mm_walk_ops swapin_walk_ops = {
	.pmd_entry		= memplus_swapin_walk_pmd_entry,
};
#endif
...
cont:
	...
	for (; addr != end; pte++, addr += PAGE_SIZE) {
		...
		// kernel 5.4 don't have below check, remove it
...
retry:
	...
	for (vma = mm->mmap; vma; vma = vma->vm_next) {
		...
		walk.private = vma;
		err = walk_page_range(vma->vm_mm, vma->vm_start, vma->vm_end, &swapin_walk_ops, &walk);
		if (err == -1)
			break;
	}
...
static inline bool should_skip(const char *comm)
{
	...
	return strnstr(comm, "camera", TASK_COMM_LEN)
		...
		|| !strncmp("captureDecision", comm, TASK_COMM_LEN)
		|| !strncmp("CameraMDMHIDL@1", comm, TASK_COMM_LEN)
